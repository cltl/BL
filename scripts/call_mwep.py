...
Options:
    ...
    --mwep_path=<mwep_path> path of mwep on machine
    --verbose=<verbose> 0 nothing, 1 descriptive stats, 2 debugging information

Example:
    python call_mwep.py --path_config_json="../config/v0.json" --mwep_path="/home/postma/multilingual-wiki-event-pipeline" --verbose="2"
"""
...
mwep_settings_path = os.path.join(settings['paths']['mwep_settings'],
                                  'config',
                                  'mwep_settings.json')
...
event_types_txt = os.path.join(settings['paths']['mwep_settings'],
                               'config',
                               'event_types.txt')
